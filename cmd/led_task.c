...
static int do_led_task_status(bool all, const char *gpio_name)
{
	struct udevice *dev;
	int banklen;
	int flags;
...
static int do_led_task(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned int gpio;
 ...
 show_usage:
	...
	if (!strncmp(str_cmd, "status", 2)) {
		...
		return cmd_process_error(cmdtp, do_led_task_status(all, str_gpio));
#else
		goto show_usage;
#endif
	}
...
U_BOOT_CMD(led_task, 4, 0, do_led_task,
